    ...
    <div className="pt-24 pb-12 px-6 min-h-screen bg-[#070e0a]">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-8">
        ...
        <div className="w-full md:w-64 space-y-2">
           ...
           <div className={`mt-4 flex items-center gap-2 px-4 py-3 rounded-xl text-xs font-semibold border ${isConfigured ? 'bg-green-500/10 border-green-500/30 text-green-400' : 'bg-red-500/10 border-red-500/30 text-red-400'}`}>
             {isConfigured
               ? <><CheckCircle2 size={14} /> Cloud Connected</>
               : <><AlertTriangle size={14} /> Local Mode</>
             }
           </div>
        </div>
        ...
        <div className="flex-1 glass p-8 rounded-[2rem] border-white/5 overflow-hidden">
          ...
          {!isConfigured && (
            <div className="flex items-start gap-3 bg-yellow-500/10 border border-yellow-500/30 text-yellow-300 rounded-xl px-5 py-4 mb-8 text-sm">
              <AlertTriangle size={18} className="shrink-0 mt-0.5" />
              <div>
                <p className="font-bold mb-0.5">Supabase Not Configured</p>
                <p className="text-yellow-300/70">Add <code className="bg-yellow-500/20 px-1 rounded">VITE_SUPABASE_URL</code> and <code className="bg-yellow-500/20 px-1 rounded">VITE_SUPABASE_ANON_KEY</code> to your <code className="bg-yellow-500/20 px-1 rounded">.env</code> file, then restart the server. Changes will only save locally until then.</p>
              </div>
            </div>
          )}
